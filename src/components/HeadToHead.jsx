...
const VsRow = ({ label, valA, valB, formatFn }) => {
    ...
    return (
        <div className="flex justify-between items-center py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors px-2 rounded-lg">
            ...
            <div className="w-1/3 text-center text-xs font-bold text-slate-400 uppercase tracking-widest leading-tight">
                {label}
            </div>
            ...
            <div className="flex items-center justify-between space-x-4 bg-slate-50 p-4 rounded-2xl border border-slate-200 shadow-inner no-print">
                <div className="flex-1">
                    ...
                    <select
                        ...
                        className="w-full bg-white border border-slate-300 text-slate-900 font-bold rounded-lg p-3"
                    >
                ...
                <div className="flex-1">
                    ...
                    <select
                        ...
                        className="w-full bg-white border border-slate-300 text-slate-900 font-bold text-right rounded-lg p-3"
                        dir="rtl"
                    >
                        {Object.keys(COLORS).map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                        ...
                        <BarChart data={chartDataH2H} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <Tooltip
                                formatter={(value, name, props) => {
                                    ...
                                    return [
                                        <span>{formatUSD(value)} <br/> <span className="text-xs text-slate-400 font-normal">Vol: {formatNumber(vol)}</span></span>,
                                        `${name}${vars ? ` (${vars})` : ''}`
                                    ];
                                }}
                                cursor={{ fill: '#f1f5f9' }}
